...
if [[ $BUILD_VERSION == "v"$1* ]]; then
    BUILD_VERSION=$BUILD_VERSION
else
    BUILD_VERSION="v$1-$BUILD_VERSION"
fi
...
cat <<EOF > src/version.rs
#![allow(non_snake_case)]
pub fn get_BUILD_VERSION() -> &'static str { "$BUILD_VERSION.e" }
pub fn get_SHORT_VERSION() -> &'static str { "$SHORT_VERSION.e" }
pub fn get_BUNDLE_VERSION() -> &'static str { "$BUNDLE_VERSION.e" }
EOF